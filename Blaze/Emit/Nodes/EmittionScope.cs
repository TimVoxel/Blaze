...
namespace Blaze.Emit.Nodes
{
    public sealed class EmittionScope
    {
        ...
        public EmittionVariableSymbol LookupOrDeclare(string name, TypeSymbol type, bool makeTemp, bool useScoping, DataLocation? location = null)
        {
        ...
        public EmittionVariableSymbol Declare(string name, TypeSymbol type, bool makeTemp, DataLocation? location = null)
        {
        ...
        {
            if (_variables == null)
                _variables = new Dictionary<string, Dictionary<DataLocation, EmittionVariableSymbol>>();

            if (!_variables.ContainsKey(emittionVariable.Name))
            {
                _variables.Add(emittionVariable.Name, new Dictionary<DataLocation, EmittionVariableSymbol>());
